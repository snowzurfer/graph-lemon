...
LightAlphaSpecMapShader::LightAlphaSpecMapShader(ID3D11Device* device, HWND hwnd, 
...
LightAlphaSpecMapShader::~LightAlphaSpecMapShader() {
...
void LightAlphaSpecMapShader::InitShader(szgrh::ConstBufManager &buf_man, 
...
void LightAlphaSpecMapShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, 
  ...
  ID3D11ShaderResourceView * texture_spec = 
    Texture::Inst()->GetTexture(mat.specular_texname.c_str());
  // Set shader texture resource in the pixel shader.
  deviceContext->PSSetShaderResources(0, 1, &texture);
  deviceContext->PSSetShaderResources(1, 1, &texture_alpha);
  deviceContext->PSSetShaderResources(2, 1, &texture_spec);
}

void LightAlphaSpecMapShader::SetShaderFrameParameters(ID3D11DeviceContext* deviceContext, std::vector<Light> &lights, Camera *cam) {
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mapped_resource;
  ...
  void LightAlphaSpecMapShader::Render(ID3D11DeviceContext* deviceContext, int indexCount)
{
