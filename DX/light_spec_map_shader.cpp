...
LightSpecMapShader::LightSpecMapShader(ID3D11Device* device, HWND hwnd, 
  ...
  InitShader(buf_man, L"../shaders/light_spec_map_vs.hlsl", 
    L"../shaders/light_spec_map_ps.hlsl", L"../shaders/tessellation_hs.hlsl",
...
void LightSpecMapShader::InitShader(sz::ConstBufManager &buf_man, 
...
void LightSpecMapShader::SetShaderFrameParameters(ID3D11DeviceContext* deviceContext, std::vector<Light> &lights, Camera *cam) {
  ...
  void LightSpecMapShader::Render(ID3D11DeviceContext* deviceContext,
    size_t index_count,
    size_t index_start,
    size_t base_vertex)
{
